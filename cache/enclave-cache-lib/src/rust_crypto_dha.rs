...
impl DHAttestation<Vec<u8>> for RustCryptoDHA {
    ...
    fn dha_session_request(&mut self, enclave_id: u32) -> Vec<u8> {
        ...
        match self.msg_format {
            MsgFormat::Json => msg_json::dha_session_request(p.0, p.1, p.2),
            MsgFormat::Protobuf => msg_proto::dha_session_request_proto(p.0, p.1, p.2),
        }
    }
    ...
    fn dha_responder_gen_msg1(&mut self, session_request: Vec<u8>, targetinfo: Vec<u8>)
        ...
        match self.msg_format {
            MsgFormat::Json => Ok(msg_json::dha_responder_gen_msg1(p.0, p.1, p.2, p.3)),
            MsgFormat::Protobuf => Ok(msg_proto::dha_responder_gen_msg1(p.0, p.1, p.2, p.3)),
        }
    }
    ...
    fn dha_initiator_proc_msg1(&mut self, msg1: Vec<u8>, enclave_id: u32) -> Result<Vec<u8>, DecodeError> {
        ...
        match self.msg_format {
            MsgFormat::Json => Ok(msg_json::dha_msg2(p.0, p.1, p.2, p.3, p.4, p.5)),
            MsgFormat::Protobuf => Ok(msg_proto::dha_msg2(p.0, p.1, p.2, p.3, p.4, p.5)),
        }
    }
    ...
    fn dha_responder_proc_msg2(&mut self, msg2: Vec<u8>) -> Result<Vec<u8>, DecodeError> {
        ...
        match self.msg_format {
            MsgFormat::Json => Ok(msg_json::dha_msg3(p.0, p.1, p.2, p.3)),
            MsgFormat::Protobuf => Ok(msg_proto::dha_msg3(p.0, p.1, p.2, p.3)),
        }
    }
