...
impl<K, V> CacheDS<K, V> where K: PartialOrd + Ord + Clone + Hash {
    ...
    pub fn get_mem_size(&self) -> usize {
        ...
        let size_map = size_of_val(&self.map);
        let size_list = size_of_val(&self.list);
        println!("Size of CacheDS: {}; Map: {}, List: {}", size, size_map, size_list);
        size
    }
...
mod test {
    ...
    fn test_size() {
        let mut cache = super::CacheDS::<String, String>::new();
        let size_a = cache.get_mem_size();
        inserts(&mut cache);
        let size_b = cache.get_mem_size();
